...
import OpenBrain.Backend.PostgreSQLBackend.UserBackend (getUser', getNobody')
...
instance InformationBackend PostgreSQLBackend where
  ...
  setChoices                  b = withWConn (conn b) setChoices'
...
setChoices' :: (IConnection conn) => conn -> InformationId -> [[InformationId]] -> IO ()
setChoices' conn iid choices = withTransaction conn $ \conn -> do
  ...
  cIds <- mapM (createCollection' conn ci) choices
